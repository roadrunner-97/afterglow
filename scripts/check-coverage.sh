...
# Begin each measurement from a clean counter set.  gcov refuses to merge
# counters generated by a newly rebuilt test executable with stale .gcda
# files, which otherwise makes a later push fail after test-source changes.
