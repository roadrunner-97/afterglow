...
void ViewportWidget::initializeGL() {
    ...
    if (!m_shader->addShaderFromSourceCode(QOpenGLShader::Vertex, VERT_SRC) ||
        !m_shader->addShaderFromSourceCode(QOpenGLShader::Fragment, FRAG_SRC) || !m_shader->link()) {
...
void ViewportWidget::paintGL() {
    ...
    if (!m_hasContent || !m_shader || !m_vao.isCreated() || !m_glTexture) return;
    ...
    if (!m_imageSize.isEmpty()) {
        pivotScreen = currentTransform().sourceToScreen(
            {m_imgPivotNorm.x() * m_imageSize.width(), m_imgPivotNorm.y() * m_imageSize.height()});
    }
    ...
    const QVector4D rectNdc((ndcX0 + ndcX1) * 0.5f, (ndcYTop + ndcYBottom) * 0.5f, (ndcX1 - ndcX0) * 0.5f,
...
void ViewportWidget::setImage(QImage image, QPoint offset) {
    ...
    glBindTexture(GL_TEXTURE_2D, m_glTexture);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, rgba.width(), rgba.height(), GL_RGBA, GL_UNSIGNED_BYTE, rgba.constBits());
    glBindTexture(GL_TEXTURE_2D, 0);
    doneCurrent();
...
void ViewportWidget::mousePressEvent(QMouseEvent *event) {
    if (m_active) {
        ...
        if (m_active->mousePress(event, vt)) {
            update();
            event->accept();
            return;
        }
...
void ViewportWidget::mouseMoveEvent(QMouseEvent *event) {
    if (m_active) {
        ...
        if (event->buttons() != Qt::NoButton) {
            if (m_active->mouseMove(event, vt)) {
                update();
                event->accept();
                return;
            }
            ...
            setCursor(c.shape() != Qt::ArrowCursor ? c
                                                   : (m_imageSize.isEmpty() ? Qt::ArrowCursor : Qt::OpenHandCursor));
            update();
            event->accept();
            return;
        }
    ...
    if (!m_panning || m_imageSize.isEmpty()) {
        event->ignore();
        return;
    }
...
void ViewportWidget::mouseReleaseEvent(QMouseEvent *event) {
    if (m_active) {
        const ViewportTransform vt = currentTransform();
        if (m_active->mouseRelease(event, vt)) {
            update();
            event->accept();
            return;
        }
