...
QString unquoteStr(const QString &token) {
    if (token.size() < 2 || !token.startsWith('"') || !token.endsWith('"')) return token; // GCOVR_EXCL_LINE
...
bool parseFromTo(const QString &val, QVariant *fromOut, QVariant *toOut) {
    ...
    for (const QString &raw : t.split(',')) {
        ...
        if (k == QStringLiteral("from")) {
            fromV   = parseScalarV(v);
            gotFrom = true;
        } else if (k == QStringLiteral("to")) {
            toV   = parseScalarV(v);
            gotTo = true;
        }
    }
        ...
        if (!e.params.isEmpty()) {
            ...
            for (auto pit = e.params.cbegin(); pit != e.params.cend(); ++pit)
                out.append("      ")
                    .append(pit.key())
                    .append(": { from: ")
                    .append(formatScalar(pit.value().from))
                    .append(", to: ")
                    .append(formatScalar(pit.value().to))
                    .append(" }\n");
        }
    }
    return out;
...
bool writeYaml(const QString &path, const QVector<UndoHistory::Entry> &entries, int cursor,
               const QVector<SettingsImporter::EffectSettings> &shadow, QString *error) {
    QFile f(path);
        ...
        if (indent == 0) {
            ...
            if (curShadow) {
                ...
                if (k == QStringLiteral("enabled")) {
                    QVariant from, to;
                    if (parseFromTo(v, &from, &to)) curEntry->enabled = {from.toBool(), to.toBool()};
            ...
            if (curShadow) {
                ...
                QVariant from, to;
                if (parseFromTo(v, &from, &to)) curEntry->params.insert(k, {from, to});
            }
        }
    }
