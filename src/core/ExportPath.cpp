...
QString resolvePattern(const QString &pattern, const QString &sourcePath, int batchIndex, QDate today) {
...
QString chooseDestination(const ExportOptions::Options &opts, const QString &sourcePath, int batchIndex,
    ...
    if (!exists) exists = [](const QString &p) { return QFile::exists(p); };
    ...
    if (!opts.subfolder.isEmpty()) {
        const QString sub = resolvePattern(opts.subfolder, sourcePath, batchIndex).trimmed();
        if (!sub.isEmpty()) baseDir = QDir(baseDir).filePath(sub);
    }
    ...
    case Policy::AppendSuffix: {
        ...
        for (int i = 1; i <= 9999; ++i) {
            const QString alt = dir.filePath(QString("%1_%2.%3").arg(stem).arg(i).arg(ext));
