...
bool HotPixelEffect::initGpuKernels(cl::Context &ctx, cl::Device &dev) {
    ...
    // GCOVR_EXCL_START
        ...
        qWarning() << "[GpuPipeline] HotPixel initGpuKernels failed:" << e.what() << "(err" << e.err() << ")";
        return false;
    }
    // GCOVR_EXCL_STOP
}

bool HotPixelEffect::enqueueGpu(cl::CommandQueue &queue, cl::Buffer &buf, cl::Buffer &aux, int w, int h,
...
HotPixelEffect::HotPixelEffect() : controlsWidget(nullptr), thresholdParam(nullptr) {}

HotPixelEffect::~HotPixelEffect() {}

QString HotPixelEffect::getName() const {
    return "Hot Pixel Removal";
}
QString HotPixelEffect::getDescription() const {
    return "Removes hot/dead pixels by replacing outliers with the local neighbourhood average";
}
QString HotPixelEffect::getVersion() const {
    return "1.0.0";
}
