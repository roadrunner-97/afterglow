...
bool BlurEffect::initGpuKernels(cl::Context &ctx, cl::Device &dev) {
    ...
    // GCOVR_EXCL_START
        ...
        qWarning() << "[GpuPipeline] Blur initGpuKernels failed:" << e.what() << "(err" << e.err() << ")";
        return false;
    }
    // GCOVR_EXCL_STOP
}

bool BlurEffect::enqueueGpu(cl::CommandQueue &queue, cl::Buffer &buf, cl::Buffer &aux, int w, int h,
...
BlurEffect::BlurEffect() : controlsWidget(nullptr), blurTypeCombo(nullptr), radiusParam(nullptr), blurType(0) {}

BlurEffect::~BlurEffect() {}

QString BlurEffect::getName() const {
    return "Blur";
}
QString BlurEffect::getDescription() const {
    return "Gaussian and box blur";
}
QString BlurEffect::getVersion() const {
    return "1.0.0";
}
