...
QString CropRotateEffect::getName() const {
    return "Crop & Rotate";
}
QString CropRotateEffect::getDescription() const {
    return "Non-destructive crop and rotation";
}
QString CropRotateEffect::getVersion() const {
    return "1.0";
}
bool CropRotateEffect::initialize() {
    return true;
}
...
QWidget *CropRotateEffect::createControlsWidget() {
    ...
    connect(m_angleSlider, &ParamSlider::editingFinished, this, [this]() { emit parametersChanged(); });
    ...
    m_lockAspectCheck->setToolTip("Constrain handle drags to preserve the current crop's aspect ratio");
    connect(m_lockAspectCheck, &QCheckBox::toggled, this, [this](bool on) { setLockAspect(on); });
...
void CropRotateEffect::applyParameters(const QMap<QString, QVariant> &p) {
    ...
    if (p.contains("cropX0") && p.contains("cropY0") && p.contains("cropX1") && p.contains("cropY1")) {
        ...
        m_userManualCrop =
            !(std::abs(x0) < eps && std::abs(y0) < eps && std::abs(x1 - 1.0) < eps && std::abs(y1 - 1.0) < eps);
    }
...
bool CropRotateEffect::enqueueGpu(cl::CommandQueue & /*queue*/, cl::Buffer & /*buf*/, cl::Buffer & /*aux*/, int /*w*/,
                                  int /*h*/, const QMap<QString, QVariant> & /*params*/) {
    return true;
}
// GCOVR_EXCL_STOP
...
QRectF CropRotateEffect::userCropRect() const {
    return m_crop;
}
...
static QPointF rotatedHalfAabbNorm(double w, double h, float angleDeg, QSize imgSize) {
    ...
    return {0.5 * (w * c + h * s / r), 0.5 * (w * s * r + h * c)};
}

QRectF CropRotateEffect::clampToImageBounds(double cx, double cy, double w, double h) const {
...
void CropRotateEffect::reFitOrClamp() {
    if (m_userManualCrop) {
        m_crop = clampToImageBounds(m_crop.center().x(), m_crop.center().y(), m_crop.width(), m_crop.height());
    } else {
...
void CropRotateEffect::setLockAspect(bool on) {
    m_lockAspect = on;
    if (on && m_imageSize.width() > 0 && m_imageSize.height() > 0 && m_crop.height() > 0.0) {
...
CropRotateEffect::Handles CropRotateEffect::buildHandles(const ViewportTransform &vt) const {
...
void CropRotateEffect::paintOverlay(QPainter &painter, const ViewportTransform &vt) {
    ...
    //   active — solid blue fill, dark border (currently being dragged)
    {
        ...
        auto activeHandleIndex = [&]() -> int {
            ...
            case DragKind::Corner:
                return m_dragIndex; // 0..3
            case DragKind::EdgeH:
                return 4 + m_dragIndex; // 4..5
            case DragKind::EdgeV:
                return 6 + m_dragIndex; // 6..7
            default:
                return -1;
            }
        };
    ...
    // grip so the user can see what they're committing to.
    {
        ...
        painter.setPen(QPen((gripActive || gripHover) ? accent : Qt::white, (gripActive || gripHover) ? 1.5 : 1.0,
        ...
        const QPointF endPt(h.rotGrip.x() + r * std::cos(endRad), h.rotGrip.y() - r * std::sin(endRad));
        ...
        QPolygonF     arrow;
        arrow << endPt << QPointF(endPt.x() + tx * t + px * w, endPt.y() + ty * t + py * w)
        ...
        if (gripActive) {
            ...
            painter.setFont(f);
            const QString text =
                QString("%1%2°").arg(m_angleDeg >= 0 ? "+" : "").arg(static_cast<double>(m_angleDeg), 0, 'f', 1);
            QFontMetrics fm(f);
            ...
            const double bx =
                toLeft ? h.rotGrip.x() - ROT_GRIP_RADIUS - pad - tb.width() - 6 : h.rotGrip.x() + ROT_GRIP_RADIUS + pad;
    ...
    if (m_subTool == SubTool::StraightenLine) {
        ...
        QRectF       strip((vt.viewportSize.width() - tb.width()) * 0.5 - 10, 8, tb.width() + 20, tb.height() + 8);
        painter.setPen(Qt::NoPen);
        painter.setBrush(QColor(0, 0, 0, 180));
...
bool CropRotateEffect::mouseMove(QMouseEvent *event, const ViewportTransform &vt) {
    ...
    if (m_dragKind == DragKind::Move) {
        ...
        m_crop =
            clampToImageBounds(static_cast<double>(x0 + dnx) + w * 0.5, static_cast<double>(y0 + dny) + h * 0.5, w, h);
        ...
        if (lockOk) {
            ...
            const double aspectN =
                m_lockedAspectPx * static_cast<double>(m_imageSize.height()) / static_cast<double>(m_imageSize.width());
            double wn = x1 - x0;
            double hn = y1 - y0;
...
bool CropRotateEffect::mouseRelease(QMouseEvent *event, const ViewportTransform &vt) {
    ...
    if (m_subTool == SubTool::StraightenLine && m_lineDrawing) {
        ...
        if (std::hypot(dx, dy) >= 4.0) {
            ...
            const float  clamped = std::clamp(static_cast<float>(quarter - lineDeg), -45.0f, 45.0f);
            m_angleDeg           = clamped;
            reFitOrClamp();
