...
// ---------------------------------------------------------------------------
static void computeWBMuls(float shotK, float targetK, float tint, float &rMul, float &gMul, float &bMul) {
...
bool WhiteBalanceEffect::initGpuKernels(cl::Context &ctx, cl::Device &dev) {
    ...
    // GCOVR_EXCL_START
        ...
        qWarning() << "[GpuPipeline] WhiteBalance initGpuKernels failed:" << e.what() << "(err" << e.err() << ")";
        return false;
    }
    // GCOVR_EXCL_STOP
}

bool WhiteBalanceEffect::enqueueGpu(cl::CommandQueue &queue, cl::Buffer &buf, cl::Buffer & /*aux*/, int w, int h,
...
WhiteBalanceEffect::WhiteBalanceEffect()
    : controlsWidget(nullptr), temperatureParam(nullptr), tintParam(nullptr), m_shotK(5500.0f) {}

WhiteBalanceEffect::~WhiteBalanceEffect() {}

QString WhiteBalanceEffect::getName() const {
    return "White Balance";
}
QString WhiteBalanceEffect::getDescription() const {
    return "Adjusts color temperature and tint";
}
QString WhiteBalanceEffect::getVersion() const {
    return "1.0.0";
}
bool WhiteBalanceEffect::initialize() {
    qDebug() << "White Balance effect initialized";
    return true;
}
...
QWidget *WhiteBalanceEffect::createControlsWidget() {
    ...
    connect(temperatureParam, &ParamSlider::editingFinished, this, [this]() { emit parametersChanged(); });
    connect(temperatureParam, &ParamSlider::valueChanged, this, [this](double) { emit liveParametersChanged(); });
    ...
    tintParam->setToolTip(
        "Fine-tunes the green–magenta axis, orthogonal to colour temperature.\nPositive = magenta shift (less green), "
        "negative = green shift.\nUse to correct fluorescent or mixed-light casts after setting temperature.");
    connect(tintParam, &ParamSlider::editingFinished, this, [this]() { emit parametersChanged(); });
    connect(tintParam, &ParamSlider::valueChanged, this, [this](double) { emit liveParametersChanged(); });
...
void WhiteBalanceEffect::applyParameters(const QMap<QString, QVariant> &parameters) {
    ...
    if (tintParam && parameters.contains("tint")) tintParam->setValue(parameters.value("tint").toDouble());
