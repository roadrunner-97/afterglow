...
static float cpuZoneEv(float L, const ZoneEvs &z) {
    ...
    return (2.0f * s3 - 3.0f * s2 + 1.0f) * pk + (s3 - 2.0f * s2 + s) * mk * h + (-2.0f * s3 + 3.0f * s2) * pk1 +
           (s3 - s2) * mk1 * h;
}
...
public:
    ...
    void setParams(const ZoneEvs &z) {
        m_z = z;
        update();
    }
...
protected:
    void paintEvent(QPaintEvent *) override {
        ...
        // Global EV handle, the diamond on the left rail
        {
            ...
            QPolygonF   diamond;
            diamond << QPointF(gp.x(), gp.y() - hs) << QPointF(gp.x() + hs, gp.y()) << QPointF(gp.x(), gp.y() + hs)
            ...
            lf.setPixelSize(8);
            p.setFont(lf);
            p.drawText(QRectF(r.left() + 14, gp.y() - 6, 20, 12), Qt::AlignLeft | Qt::AlignVCenter, "EV");
        }
        ...
        if (std::abs(m_z.global) >= 0.05f) {
            ...
            QString label = QString("Base: %1%2 EV").arg(m_z.global > 0 ? "+" : "").arg(double(m_z.global), 0, 'f', 1);
...
ExposureEffect::ExposureEffect()
    : controlsWidget(nullptr), exposureParam(nullptr), whitesParam(nullptr), highlightsParam(nullptr),
...
QWidget *ExposureEffect::createControlsWidget() {
    ...
    curve->setToolTip(
        "Live tone curve showing the zone sliders' effect.\nThe white line is the zone response; the dashed line is "
        "neutral.\nThe grey fill is the luminance histogram, shifted rightward by the Exposure slider to show where "
        "pixels land on the adjusted image.\nZone bands from left to right: Blacks, Shadows, Highlights, Whites.");
...
bool ExposureEffect::initGpuKernels(cl::Context &ctx, cl::Device &dev) {
    ...
    // GCOVR_EXCL_START
        ...
        qWarning() << "[GpuPipeline] Exposure initGpuKernels failed:" << e.what() << "(err" << e.err() << ")";
        return false;
    }
    // GCOVR_EXCL_STOP
}

bool ExposureEffect::enqueueGpu(cl::CommandQueue &queue, cl::Buffer &buf, cl::Buffer & /*aux*/, int w, int h,
    ...
    if (globalEv == 0.0f && blacksEv == 0.0f && shadowsEv == 0.0f && highlightsEv == 0.0f && whitesEv == 0.0f) {
        return true; // no-op
    }
