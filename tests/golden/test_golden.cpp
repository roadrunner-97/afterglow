...
private slots:
    void initTestCase() {
        ...
        if (GpuDeviceRegistry::instance().count() == 0) QSKIP("No OpenCL device found — skipping golden-image tests");
    ...
    void run() {
        ...
        if (caseDir.isEmpty()) QSKIP("No cases under " GOLDEN_CASES_DIR " — drop fixtures in to enable");
        ...
        QVERIFY2(QFile::exists(settingsPath), qPrintable("missing settings.yaml/yml in " + caseDir));
        ...
        if (RawLoader::isRawFile(input.fileName())) {
            ...
            QVERIFY2(!src.isNull(), qPrintable("could not load input " + input.absoluteFilePath()));
        }
        ...
        for (const auto &e : m_effects.entries()) {
            ...
            if (auto *cs = dynamic_cast<ICropSource *>(e.effect)) cs->setSourceImageSize(src.size());
        }
        ...
        QVERIFY2(!exp.isNull(), qPrintable("could not load " + expected.absoluteFilePath()));
