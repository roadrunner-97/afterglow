...
class TestSplitToning : public GpuTestBase {
    Q_OBJECT
...
private slots:
    ...
    void shadowTint_biasesDarkPixelsTowardHue() {
        ...
        QVERIFY2(b > r && b > g, qPrintable(QString("r=%1 g=%2 b=%3").arg(r).arg(g).arg(b)));
    }
    ...
    void highlightTint_biasesBrightPixelsTowardHue() {
        ...
        int g = pixelG(out, 32, 32);
        int b = pixelB(out, 32, 32);
        QVERIFY2(r > g && r > b, qPrintable(QString("r=%1 g=%2 b=%3").arg(r).arg(g).arg(b)));
    }
