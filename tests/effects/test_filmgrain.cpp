...
private slots:
    ...
    void nonZeroAmount_perturbsMidtone() {
        ...
        QVERIFY2(changed > 64 * 64 * 0.9, qPrintable(QString("changed=%1").arg(changed)));
    }
    ...
    void grainSize_smoothNoiseNotBlocky() {
        ...
        QVERIFY2(distinct > 64 * 32, qPrintable(QString("distinct=%1").arg(distinct)));
        ...
        QVERIFY2(maxDelta < 80, qPrintable(QString("maxDelta=%1").arg(maxDelta)));
    }
    ...
    void seed_affectsOutput() {
        ...
        QMap<QString, QVariant> p0 = base;
        p0["seed"]                 = 0;
        QMap<QString, QVariant> p1 = base;
        p1["seed"]                 = 1;
    ...
    void nonSquare_nonZeroAmount_perturbsMidtone() {
        ...
        int changed = differingPixels(out, qRgb(128, 128, 128));
        QVERIFY2(changed > 128 * 64 * 0.9, qPrintable(QString("changed=%1").arg(changed)));
    }
