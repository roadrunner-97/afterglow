...
class TestGrayscale : public GpuTestBase {
    Q_OBJECT
...
private slots:
    ...
    void active_convertsToGray() {
        ...
        QVERIFY(allPixels(out, [](QRgb px) { return qRed(px) == qGreen(px) && qGreen(px) == qBlue(px); }));
    }
    ...
    void active_greyValueMatchesLuminosity() {
        ...
        QVERIFY2(qAbs(grey - 128) <= 2, qPrintable(QString("expected ~128, got %1").arg(grey)));
    }
    ...
    void active_16bit() {
        ...
        QVERIFY(allPixels(out32, [](QRgb px) { return qRed(px) == qGreen(px) && qGreen(px) == qBlue(px); }));
    }
